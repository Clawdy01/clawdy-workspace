...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred-forty-eight'),
    ('driehonderdnegenenzestig', 'vierhonderdachtenveertig'),
    ('all_cases[:364]', 'all_cases[:433]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:433]'),
    ('!= 364', '!= 433'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred-forty-eight')
    text = src.read_text()
