...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred-forty'),
    ('driehonderdnegenenzestig', 'vierhonderdveertig'),
    ('all_cases[:364]', 'all_cases[:425]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:425]'),
    ('!= 364', '!= 425'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred-forty')
    text = src.read_text()
