...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred-sixty-six'),
    ('driehonderdnegenenzestig', 'vierhonderdzesenzestig'),
    ('all_cases[:364]', 'all_cases[:451]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:451]'),
    ('!= 364', '!= 451'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred-sixty-six')
    text = src.read_text()
