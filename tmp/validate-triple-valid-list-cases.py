...
valid_cases = all_cases[:3]
if len(valid_cases) != 3:
    raise SystemExit(f'kon geen drie geldige casenamen vinden, kreeg {valid_cases}')
...
json_run_proc = run('--json', *case_args)
if json_run_proc.returncode != 0:
    raise SystemExit(f'json run exitcode verwacht 0, kreeg {json_run_proc.returncode}')
if json_run_proc.stderr.strip():
    raise SystemExit(f'json run hoort geen stderr te geven, kreeg: {json_run_proc.stderr}')
json_run_payload = json.loads(json_run_proc.stdout)
if json_run_payload.get('requested_case_names') != expected_requested:
    raise SystemExit(
        'json run requested_case_names mismatch: '
        f'{json_run_payload.get("requested_case_names")} != {expected_requested}'
    )
if json_run_payload.get('requested_case_count') != len(expected_requested):
    raise SystemExit(
        'json run requested_case_count mismatch: '
        f'{json_run_payload.get("requested_case_count")} != {len(expected_requested)}'
    )
if json_run_payload.get('selected_case_names') != expected_requested:
    raise SystemExit(
        'json run selected_case_names mismatch: '
        f'{json_run_payload.get("selected_case_names")} != {expected_requested}'
    )
if json_run_payload.get('selected_case_count') != len(expected_requested):
    raise SystemExit(
        'json run selected_case_count mismatch: '
        f'{json_run_payload.get("selected_case_count")} != {len(expected_requested)}'
    )
if json_run_payload.get('available_case_names') != all_cases:
    raise SystemExit('json run available_case_names mismatch met volledige --list-cases uitvoer')
if json_run_payload.get('available_case_count') != len(all_cases):
    raise SystemExit(
        'json run available_case_count mismatch: '
        f'{json_run_payload.get("available_case_count")} != {len(all_cases)}'
    )
if json_run_payload.get('ok') is not True:
    raise SystemExit(f'json run ok verwacht True, kreeg {json_run_payload.get("ok")}')
