...
valid_cases = [case_name for case_name in all_cases if case_name not in {UNKNOWN, TYPO}][:367]
if len(valid_cases) != 367:
    raise SystemExit(f'kon geen driehonderdtweeënzeventig geldige casenamen vinden, kreeg {valid_cases}')
...
if expected_stderr not in (plain_proc.stderr or ''):
    raise SystemExit('plain stderr noemt niet alle driehonderdtweeënzeventig geldige first-seen cases')
