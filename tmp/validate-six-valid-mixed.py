...
valid_cases = [case_name for case_name in all_cases if case_name not in {UNKNOWN, TYPO}][:6]
if len(valid_cases) != 6:
    raise SystemExit(f'kon geen zes geldige casenamen vinden, kreeg {valid_cases}')
...
expected_selected = [valid_cases[5], valid_cases[2], valid_cases[4], valid_cases[1], valid_cases[3], valid_cases[0]]
...
if expected_stderr not in (plain_proc.stderr or ''):
    raise SystemExit('plain stderr noemt niet alle zes geldige first-seen cases')
