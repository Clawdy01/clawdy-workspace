...
repls = [
    ('four-hundred-fifty-eight', 'four-hundred-sixty-one'),
    ('vierhonderdachtenvijftig', 'vierhonderdeenenzestig'),
    ('all_cases[:443]', 'all_cases[:446]'),
    ('{UNKNOWN, TYPO}][:443]', '{UNKNOWN, TYPO}][:446]'),
    ('!= 443', '!= 446'),
    ('kreeg 443', 'kreeg 446'),
...
for src_name in [
    ...
    src = root / src_name
    dst = root / src_name.replace('four-hundred-fifty-eight', 'four-hundred-sixty-one')
    text = src.read_text()
