...
valid_cases = [case_name for case_name in all_cases if case_name not in {UNKNOWN, TYPO}][:563]
if len(valid_cases) != 563:
    raise SystemExit(f'kon geen vijfhonderdachtenzeventig geldige casenamen vinden, kreeg {valid_cases}')
...
if expected_stderr not in (plain_proc.stderr or ''):
    raise SystemExit('plain stderr noemt niet alle vijfhonderddrieënzestig geldige first-seen cases')
