...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred-sixty-three'),
    ('driehonderdnegenenzestig', 'vierhonderddrieënzestig'),
    ('all_cases[:364]', 'all_cases[:448]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:448]'),
    ('!= 364', '!= 448'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred-sixty-three')
    text = src.read_text()
