...
valid_cases = [case_name for case_name in all_cases if case_name not in {UNKNOWN, TYPO}][:615]
if len(valid_cases) != 615:
    raise SystemExit(f'kon geen zeshonderddertig geldige casenamen vinden, kreeg {valid_cases}')
...
if expected_stderr not in (plain_proc.stderr or ''):
    raise SystemExit('plain stderr noemt niet alle zeshonderdvijftien geldige first-seen cases')
