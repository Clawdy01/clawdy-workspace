...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred-twenty-nine'),
    ('driehonderdnegenenzestig', 'vierhonderdnegenentwintig'),
    ('all_cases[:364]', 'all_cases[:414]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:414]'),
    ('!= 364', '!= 414'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred-twenty-nine')
    text = src.read_text()
