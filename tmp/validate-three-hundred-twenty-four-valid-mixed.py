...
valid_cases = [case_name for case_name in all_cases if case_name not in {UNKNOWN, TYPO}][:320]
if len(valid_cases) != 320:
    raise SystemExit(f'kon geen driehonderdvierentwintig geldige casenamen vinden, kreeg {valid_cases}')
...
if expected_stderr not in (plain_proc.stderr or ''):
    raise SystemExit('plain stderr noemt niet alle driehonderdvierentwintig geldige first-seen cases')
