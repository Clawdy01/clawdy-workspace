...
valid_cases = [case_name for case_name in all_cases if case_name not in {UNKNOWN, TYPO}][:105]
if len(valid_cases) != 105:
    raise SystemExit(f'kon geen honderdvijf geldige casenamen vinden, kreeg {valid_cases}')
...
if expected_stderr not in (plain_proc.stderr or ''):
    raise SystemExit('plain stderr noemt niet alle honderdvijf geldige first-seen cases')
