...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred-three'),
    ('driehonderdnegenenzestig', 'vierhonderddrie'),
    ('all_cases[:364]', 'all_cases[:398]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:398]'),
    ('!= 364', '!= 398'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred-three')
    text = src.read_text()
