...
repls = [
    ('three-hundred-sixty-one', 'four-hundred-nineteen'),
    ('driehonderdeenenzestig', 'vierhonderdnegentien'),
    ('all_cases[:356]', 'all_cases[:404]'),
    ('{UNKNOWN, TYPO}][:356]', '{UNKNOWN, TYPO}][:404]'),
    ('!= 356', '!= 404'),
...
for kind in ('valid-list-cases', 'valid-mixed'):
    ...
    dst = root / 'tmp' / f'validate-four-hundred-nineteen-{kind}.py'
    text = src.read_text()
