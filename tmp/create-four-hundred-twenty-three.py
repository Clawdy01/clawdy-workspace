...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred-twenty-three'),
    ('driehonderdnegenenzestig', 'vierhonderddrieentwintig'),
    ('all_cases[:364]', 'all_cases[:408]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:408]'),
    ('!= 364', '!= 408'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred-twenty-three')
    text = src.read_text()
