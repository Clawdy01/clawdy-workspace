...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred-twenty-five'),
    ('driehonderdnegenenzestig', 'vierhonderdvijfentwintig'),
    ('all_cases[:364]', 'all_cases[:410]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:410]'),
    ('!= 364', '!= 410'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred-twenty-five')
    text = src.read_text()
