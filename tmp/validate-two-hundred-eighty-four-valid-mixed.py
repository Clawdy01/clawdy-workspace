...
valid_cases = [case_name for case_name in all_cases if case_name not in {UNKNOWN, TYPO}][:280]
if len(valid_cases) != 280:
    raise SystemExit(f'kon geen tweehonderdvierentachtig geldige casenamen vinden, kreeg {valid_cases}')
...
if expected_stderr not in (plain_proc.stderr or ''):
    raise SystemExit('plain stderr noemt niet alle tweehonderdvierentachtig geldige first-seen cases')
