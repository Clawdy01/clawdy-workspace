...
repls = [
    ('four-hundred-fifty-eight', 'four-hundred-sixty-two'),
    ('vierhonderdachtenvijftig', 'vierhonderdtweeënzestig'),
    ('all_cases[:443]', 'all_cases[:447]'),
    ('{UNKNOWN, TYPO}][:443]', '{UNKNOWN, TYPO}][:447]'),
    ('!= 443', '!= 447'),
    ('kreeg 443', 'kreeg 447'),
...
for src_name in [
    ...
    src = root / src_name
    dst = root / src_name.replace('four-hundred-fifty-eight', 'four-hundred-sixty-two')
    text = src.read_text()
