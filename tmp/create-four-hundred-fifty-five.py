...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred-fifty-five'),
    ('driehonderdnegenenzestig', 'vierhonderdvijfenvijftig'),
    ('all_cases[:364]', 'all_cases[:440]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:440]'),
    ('!= 364', '!= 440'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred-fifty-five')
    text = src.read_text()
