...
repls = [
    ('three-hundred-sixty-nine', 'three-hundred-ninety-eight'),
    ('driehonderdnegenenzestig', 'driehonderdachtennegentig'),
    ('all_cases[:364]', 'all_cases[:393]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:393]'),
    ('!= 364', '!= 393'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'three-hundred-ninety-eight')
    text = src.read_text()
