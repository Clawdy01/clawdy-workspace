...
valid_cases = [case_name for case_name in all_cases if case_name not in {UNKNOWN, TYPO}][:325]
if len(valid_cases) != 325:
    raise SystemExit(f'kon geen driehonderdnegenentwintig geldige casenamen vinden, kreeg {valid_cases}')
...
if expected_stderr not in (plain_proc.stderr or ''):
    raise SystemExit('plain stderr noemt niet alle driehonderdnegenentwintig geldige first-seen cases')
