...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred-fifty-two'),
    ('driehonderdnegenenzestig', 'vierhonderdtweeënvijftig'),
    ('all_cases[:364]', 'all_cases[:437]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:437]'),
    ('!= 364', '!= 437'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred-fifty-two')
    text = src.read_text()
