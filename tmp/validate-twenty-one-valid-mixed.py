...
ORDER = [20, 17, 19, 15, 18, 13, 16, 11, 14, 9, 12, 7, 10, 5, 8, 3, 6, 1, 4, 2, 0]
...
valid_cases = [case_name for case_name in all_cases if case_name not in {UNKNOWN, TYPO}][:21]
if len(valid_cases) != 21:
    raise SystemExit(f'kon geen eenentwintig geldige casenamen vinden, kreeg {valid_cases}')
...
if expected_stderr not in (plain_proc.stderr or ''):
    raise SystemExit('plain stderr noemt niet alle eenentwintig geldige first-seen cases')
