...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred-nineteen'),
    ('driehonderdnegenenzestig', 'vierhonderdnegentien'),
    ('all_cases[:364]', 'all_cases[:404]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:404]'),
    ('!= 364', '!= 404'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred-nineteen')
    text = src.read_text()
