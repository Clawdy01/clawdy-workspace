...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred-forty-five'),
    ('driehonderdnegenenzestig', 'vierhonderdvijfenveertig'),
    ('all_cases[:364]', 'all_cases[:430]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:430]'),
    ('!= 364', '!= 430'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred-forty-five')
    text = src.read_text()
