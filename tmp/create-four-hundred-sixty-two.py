...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred-sixty-two'),
    ('driehonderdnegenenzestig', 'vierhonderdtweeënzestig'),
    ('all_cases[:364]', 'all_cases[:447]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:447]'),
    ('!= 364', '!= 447'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred-sixty-two')
    text = src.read_text()
