...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred-thirty-eight'),
    ('driehonderdnegenenzestig', 'vierhonderdachtendertig'),
    ('all_cases[:364]', 'all_cases[:423]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:423]'),
    ('!= 364', '!= 423'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred-thirty-eight')
    text = src.read_text()
