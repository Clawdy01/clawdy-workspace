...
repls = [
    ('three-hundred-sixty-nine', 'three-hundred-ninety-four'),
    ('driehonderdnegenenzestig', 'driehonderdvierennegentig'),
    ('all_cases[:364]', 'all_cases[:389]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:389]'),
    ('!= 364', '!= 389'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'three-hundred-ninety-four')
    text = src.read_text()
