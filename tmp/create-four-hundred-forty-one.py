...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred-forty-one'),
    ('driehonderdnegenenzestig', 'vierhonderdeenenveertig'),
    ('all_cases[:364]', 'all_cases[:426]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:426]'),
    ('!= 364', '!= 426'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred-forty-one')
    text = src.read_text()
