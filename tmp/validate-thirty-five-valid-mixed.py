...
ORDER = [34, 33, 32, 31, 30, 29, 26, 28, 24, 27, 22, 25, 20, 23, 18, 21, 16, 19, 14, 17, 12, 15, 10, 13, 8, 11, 6, 9, 4, 7, 2, 5, 0, 3, 1]
...
valid_cases = [case_name for case_name in all_cases if case_name not in {UNKNOWN, TYPO}][:35]
if len(valid_cases) != 35:
    raise SystemExit(f'kon geen vijfendertig geldige casenamen vinden, kreeg {valid_cases}')
...
if expected_stderr not in (plain_proc.stderr or ''):
    raise SystemExit('plain stderr noemt niet alle vijfendertig geldige first-seen cases')
