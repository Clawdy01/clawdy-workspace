...
ORDER = [16, 13, 15, 11, 14, 9, 12, 7, 10, 5, 8, 3, 6, 1, 4, 0, 2]
...
valid_cases = [case_name for case_name in all_cases if case_name not in {UNKNOWN, TYPO}][:17]
if len(valid_cases) != 17:
    raise SystemExit(f'kon geen zeventien geldige casenamen vinden, kreeg {valid_cases}')
...
if expected_stderr not in (plain_proc.stderr or ''):
    raise SystemExit('plain stderr noemt niet alle zeventien geldige first-seen cases')
