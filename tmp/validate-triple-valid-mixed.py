...
valid_cases = [case_name for case_name in all_cases if case_name not in {UNKNOWN, TYPO}][:3]
...
expected_selected = [valid_cases[2], valid_cases[0], valid_cases[1]]
...
if expected_stderr not in (plain_proc.stderr or ''):
    raise SystemExit('plain stderr noemt niet alle drie geldige first-seen cases')
