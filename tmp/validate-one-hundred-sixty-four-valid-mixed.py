...
valid_cases = [case_name for case_name in all_cases if case_name not in {UNKNOWN, TYPO}][:164]
if len(valid_cases) != 164:
    raise SystemExit(f'kon geen honderdvierenzestig geldige casenamen vinden, kreeg {valid_cases}')
...
if expected_stderr not in (plain_proc.stderr or ''):
    raise SystemExit('plain stderr noemt niet alle honderdvierenzestig geldige first-seen cases')
