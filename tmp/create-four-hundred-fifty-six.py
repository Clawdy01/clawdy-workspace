...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred-fifty-six'),
    ('driehonderdnegenenzestig', 'vierhonderdzesenvijftig'),
    ('all_cases[:364]', 'all_cases[:441]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:441]'),
    ('!= 364', '!= 441'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred-fifty-six')
    text = src.read_text()
