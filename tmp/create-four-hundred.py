...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred'),
    ('driehonderdnegenenzestig', 'vierhonderd'),
    ('all_cases[:364]', 'all_cases[:395]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:395]'),
    ('!= 364', '!= 395'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred')
    text = src.read_text()
