...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred-thirty'),
    ('driehonderdnegenenzestig', 'vierhonderddertig'),
    ('all_cases[:364]', 'all_cases[:415]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:415]'),
    ('!= 364', '!= 415'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred-thirty')
    text = src.read_text()
