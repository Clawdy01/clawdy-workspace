...
valid_cases = [case_name for case_name in all_cases if case_name not in {UNKNOWN, TYPO}][:165]
if len(valid_cases) != 165:
    raise SystemExit(f'kon geen honderdvijfenzestig geldige casenamen vinden, kreeg {valid_cases}')
...
if expected_stderr not in (plain_proc.stderr or ''):
    raise SystemExit('plain stderr noemt niet alle honderdvijfenzestig geldige first-seen cases')
