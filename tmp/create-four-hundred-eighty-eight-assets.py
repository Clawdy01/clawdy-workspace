...
repls = [
    ('four-hundred-sixty-three', 'four-hundred-eighty-eight'),
    ('vierhonderddrieënzestig', 'vierhonderdachtentachtig'),
    ('[:448]', '[:473]'),
    ('!= 448', '!= 473'),
    ('kreeg 448', 'kreeg 473'),
]
for name in files:
    ...
    if name in {
        ...
        new = ', 443, 444, 445, 446, 447, 448, 449, 450, 451, 452, 453, 454, 455, 456, 457, 458, 459, 460, 461, 462, 463, 464, 465]'
        if old not in text:
            raise SystemExit(f'mis expected ORDER tail in {name}')
        text = text.replace(old, new, 1)
    (root / name.replace('four-hundred-sixty-three', 'four-hundred-eighty-eight')).write_text(text)
print('created')
