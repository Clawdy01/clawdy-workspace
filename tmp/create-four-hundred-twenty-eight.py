...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred-twenty-eight'),
    ('driehonderdnegenenzestig', 'vierhonderdachtentwintig'),
    ('all_cases[:364]', 'all_cases[:413]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:413]'),
    ('!= 364', '!= 413'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred-twenty-eight')
    text = src.read_text()
