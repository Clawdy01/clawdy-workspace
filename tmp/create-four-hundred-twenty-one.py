...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred-twenty-one'),
    ('driehonderdnegenenzestig', 'vierhonderdeenentwintig'),
    ('all_cases[:364]', 'all_cases[:406]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:406]'),
    ('!= 364', '!= 406'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred-twenty-one')
    text = src.read_text()
