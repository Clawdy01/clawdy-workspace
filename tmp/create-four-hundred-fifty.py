...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred-fifty'),
    ('driehonderdnegenenzestig', 'vierhonderdvijftig'),
    ('all_cases[:364]', 'all_cases[:435]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:435]'),
    ('!= 364', '!= 435'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred-fifty')
    text = src.read_text()
