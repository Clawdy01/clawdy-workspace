...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred-sixty-five'),
    ('driehonderdnegenenzestig', 'vierhonderdvijfenzestig'),
    ('all_cases[:364]', 'all_cases[:450]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:450]'),
    ('!= 364', '!= 450'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred-sixty-five')
    text = src.read_text()
