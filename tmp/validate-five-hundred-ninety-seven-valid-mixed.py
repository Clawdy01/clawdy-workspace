...
valid_cases = [case_name for case_name in all_cases if case_name not in {UNKNOWN, TYPO}][:582]
if len(valid_cases) != 582:
    raise SystemExit(f'kon geen vijfhonderdzevenennegentig geldige casenamen vinden, kreeg {valid_cases}')
...
if expected_stderr not in (plain_proc.stderr or ''):
    raise SystemExit('plain stderr noemt niet alle vijfhonderdtweeëntachtig geldige first-seen cases')
