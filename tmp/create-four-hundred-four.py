...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred-four'),
    ('driehonderdnegenenzestig', 'vierhonderdvier'),
    ('all_cases[:364]', 'all_cases[:399]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:399]'),
    ('!= 364', '!= 399'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred-four')
    text = src.read_text()
