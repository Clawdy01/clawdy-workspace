...
valid_cases = [case_name for case_name in all_cases if case_name not in {UNKNOWN, TYPO}][:117]
if len(valid_cases) != 117:
    raise SystemExit(f'kon geen honderdzeventien geldige casenamen vinden, kreeg {valid_cases}')
...
if expected_stderr not in (plain_proc.stderr or ''):
    raise SystemExit('plain stderr noemt niet alle honderdzeventien geldige first-seen cases')
