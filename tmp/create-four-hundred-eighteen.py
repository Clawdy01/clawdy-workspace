...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred-eighteen'),
    ('driehonderdnegenenzestig', 'vierhonderdachttien'),
    ('all_cases[:364]', 'all_cases[:403]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:403]'),
    ('!= 364', '!= 403'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred-eighteen')
    text = src.read_text()
