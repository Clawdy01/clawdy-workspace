...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred-thirty-one'),
    ('driehonderdnegenenzestig', 'vierhonderdeenendertig'),
    ('all_cases[:364]', 'all_cases[:416]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:416]'),
    ('!= 364', '!= 416'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred-thirty-one')
    text = src.read_text()
