...
valid_cases = [case_name for case_name in all_cases if case_name not in {UNKNOWN, TYPO}][:230]
if len(valid_cases) != 230:
    raise SystemExit(f'kon geen tweehonderdtweeëndertig geldige casenamen vinden, kreeg {valid_cases}')
...
if expected_stderr not in (plain_proc.stderr or ''):
    raise SystemExit('plain stderr noemt niet alle tweehonderdtweeëndertig geldige first-seen cases')
