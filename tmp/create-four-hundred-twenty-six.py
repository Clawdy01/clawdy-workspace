...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred-twenty-six'),
    ('driehonderdnegenenzestig', 'vierhonderdzesentwintig'),
    ('all_cases[:364]', 'all_cases[:411]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:411]'),
    ('!= 364', '!= 411'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred-twenty-six')
    text = src.read_text()
