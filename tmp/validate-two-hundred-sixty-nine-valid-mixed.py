...
valid_cases = [case_name for case_name in all_cases if case_name not in {UNKNOWN, TYPO}][:265]
if len(valid_cases) != 265:
    raise SystemExit(f'kon geen tweehonderdnegenenzestig geldige casenamen vinden, kreeg {valid_cases}')
...
if expected_stderr not in (plain_proc.stderr or ''):
    raise SystemExit('plain stderr noemt niet alle tweehonderdnegenenzestig geldige first-seen cases')
