...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred-forty-seven'),
    ('driehonderdnegenenzestig', 'vierhonderdzevenenveertig'),
    ('all_cases[:364]', 'all_cases[:432]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:432]'),
    ('!= 364', '!= 432'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred-forty-seven')
    text = src.read_text()
