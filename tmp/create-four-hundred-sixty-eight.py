...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred-sixty-eight'),
    ('driehonderdnegenenzestig', 'vierhonderdachtenzestig'),
    ('all_cases[:364]', 'all_cases[:453]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:453]'),
    ('!= 364', '!= 453'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred-sixty-eight')
    text = src.read_text()
