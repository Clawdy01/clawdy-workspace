...
repls = [
    ('three-hundred-sixty-nine', 'three-hundred-ninety-three'),
    ('driehonderdnegenenzestig', 'driehonderddrieënnegentig'),
    ('all_cases[:364]', 'all_cases[:388]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:388]'),
    ('!= 364', '!= 388'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'three-hundred-ninety-three')
    text = src.read_text()
