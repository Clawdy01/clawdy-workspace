...
repls = [
    ('three-hundred-sixty-nine', 'three-hundred-ninety-seven'),
    ('driehonderdnegenenzestig', 'driehonderdzevenennegentig'),
    ('all_cases[:364]', 'all_cases[:392]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:392]'),
    ('!= 364', '!= 392'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'three-hundred-ninety-seven')
    text = src.read_text()
