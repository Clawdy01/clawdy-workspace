...
valid_cases = [case_name for case_name in all_cases if case_name not in {UNKNOWN, TYPO}][:341]
if len(valid_cases) != 341:
    raise SystemExit(f'kon geen driehonderdvijfenveertig geldige casenamen vinden, kreeg {valid_cases}')
...
if expected_stderr not in (plain_proc.stderr or ''):
    raise SystemExit('plain stderr noemt niet alle driehonderdvijfenveertig geldige first-seen cases')
