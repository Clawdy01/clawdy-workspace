...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred-thirty-three'),
    ('driehonderdnegenenzestig', 'vierhonderddrieendertig'),
    ('all_cases[:364]', 'all_cases[:418]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:418]'),
    ('!= 364', '!= 418'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred-thirty-three')
    text = src.read_text()
