...
valid_cases = [case_name for case_name in all_cases if case_name not in {UNKNOWN, TYPO}][:150]
if len(valid_cases) != 150:
    raise SystemExit(f'kon geen honderdvijftig geldige casenamen vinden, kreeg {valid_cases}')
...
if expected_stderr not in (plain_proc.stderr or ''):
    raise SystemExit('plain stderr noemt niet alle honderdvijftig geldige first-seen cases')
