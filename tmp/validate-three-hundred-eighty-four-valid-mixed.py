...
valid_cases = [case_name for case_name in all_cases if case_name not in {UNKNOWN, TYPO}][:379]
if len(valid_cases) != 379:
    raise SystemExit(f'kon geen driehonderdvierentachtig geldige casenamen vinden, kreeg {valid_cases}')
...
if expected_stderr not in (plain_proc.stderr or ''):
    raise SystemExit('plain stderr noemt niet alle driehonderdvierentachtig geldige first-seen cases')
