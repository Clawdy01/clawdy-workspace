...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred-five'),
    ('driehonderdnegenenzestig', 'vierhonderdvijf'),
    ('all_cases[:364]', 'all_cases[:400]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:400]'),
    ('!= 364', '!= 400'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred-five')
    text = src.read_text()
