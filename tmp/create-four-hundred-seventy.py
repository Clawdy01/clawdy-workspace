...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred-seventy'),
    ('driehonderdnegenenzestig', 'vierhonderdzeventig'),
    ('all_cases[:364]', 'all_cases[:455]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:455]'),
    ('!= 364', '!= 455'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred-seventy')
    text = src.read_text()
