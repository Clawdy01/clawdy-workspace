...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred-thirty-nine'),
    ('driehonderdnegenenzestig', 'vierhonderdnegenendertig'),
    ('all_cases[:364]', 'all_cases[:424]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:424]'),
    ('!= 364', '!= 424'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred-thirty-nine')
    text = src.read_text()
