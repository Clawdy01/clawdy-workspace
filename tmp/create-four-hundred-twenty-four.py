...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred-twenty-four'),
    ('driehonderdnegenenzestig', 'vierhonderdvierentwintig'),
    ('all_cases[:364]', 'all_cases[:409]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:409]'),
    ('!= 364', '!= 409'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred-twenty-four')
    text = src.read_text()
