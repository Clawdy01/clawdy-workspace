#!/usr/bin/env python3
from pathlib import Path

ROOT = Path('/home/clawdy/.openclaw/workspace/tmp')


def replace_once(text: str, old: str, new: str, label: str) -> str:
    if old not in text:
        raise SystemExit(f'missing in {label}: {old}')
    return text.replace(old, new, 1)


def build(src: str, dst: str, replacements: list[tuple[str, str]]) -> None:
    text = (ROOT / src).read_text()
    for old, new in replacements:
        text = replace_once(text, old, new, src)
    (ROOT / dst).write_text(text)
    print(dst)


build(
    'create-five-hundred-twenty-six-assets.py',
    'create-five-hundred-twenty-seven-assets.py',
    [
        ('five-hundred-twenty-six', 'five-hundred-twenty-seven'),
        ('vijfhonderdzesentwintig', 'vijfhonderdzevenentwintig'),
        ('[:511]', '[:512]'),
        ('!= 511', '!= 512'),
        ('kreeg 511', 'kreeg 512'),
        ('498, 499, 500, 501, 502', '498, 499, 500, 501, 502, 503'),
    ],
)

build(
    'create-five-hundred-twenty-six-bootstrap.py',
    'create-five-hundred-twenty-seven-bootstrap.py',
    [
        ('five-hundred-twenty-six', 'five-hundred-twenty-seven'),
        ('vijfhonderdzesentwintig', 'vijfhonderdzevenentwintig'),
        ('all_cases[:511]', 'all_cases[:512]'),
        ('{UNKNOWN, TYPO}][:511]', '{UNKNOWN, TYPO}][:512]'),
        ('!= 511', '!= 512'),
        ('kreeg 511', 'kreeg 512'),
        ('494, 495, 496, 497, 498', '494, 495, 496, 497, 498, 499'),
    ],
)

build(
    'create-five-hundred-twenty-six-minimal.py',
    'create-five-hundred-twenty-seven-minimal.py',
    [
        ('five-hundred-twenty-six', 'five-hundred-twenty-seven'),
        ('vijfhonderdzesentwintig', 'vijfhonderdzevenentwintig'),
        ('!= 511', '!= 512'),
        ('kreeg 511', 'kreeg 512'),
        (' 452)', ' 453)'),
        ('494, 495, 496, 497, 498', '494, 495, 496, 497, 498, 499'),
    ],
)

build(
    'make-five-hundred-twenty-six.py',
    'make-five-hundred-twenty-seven.py',
    [
        ('five-hundred-twenty-six', 'five-hundred-twenty-seven'),
        ('vijfhonderdzesentwintig', 'vijfhonderdzevenentwintig'),
        ('all_cases[:511]', 'all_cases[:512]'),
        ('{UNKNOWN, TYPO}][:511]', '{UNKNOWN, TYPO}][:512]'),
        ('!= 511', '!= 512'),
        ('434, 435, 436, 437, 438', '434, 435, 436, 437, 438, 439'),
    ],
)

build(
    'create-five-hundred-twenty-six-files.py',
    'create-five-hundred-twenty-seven-files.py',
    [
        ('five-hundred-twenty-six', 'five-hundred-twenty-seven'),
        ('vijfhonderdzesentwintig', 'vijfhonderdzevenentwintig'),
        ('all_cases[:511]', 'all_cases[:512]'),
        ('{UNKNOWN, TYPO}][:511]', '{UNKNOWN, TYPO}][:512]'),
        ('!= 511', '!= 512'),
        ('434, 435, 436, 437, 438', '434, 435, 436, 437, 438, 439'),
    ],
)

build(
    'create-five-hundred-twenty-six.py',
    'create-five-hundred-twenty-seven.py',
    [
        ('five-hundred-twenty-six', 'five-hundred-twenty-seven'),
        ('vijfhonderdzesentwintig', 'vijfhonderdzevenentwintig'),
        ('all_cases[:511]', 'all_cases[:512]'),
        ('{UNKNOWN, TYPO}][:511]', '{UNKNOWN, TYPO}][:512]'),
        ('!= 511', '!= 512'),
        ('437, 438, 439, 440, 441', '437, 438, 439, 440, 441, 442'),
    ],
)

build(
    'generate-validate-five-hundred-twenty-six.py',
    'generate-validate-five-hundred-twenty-seven.py',
    [
        ('five-hundred-twenty-six', 'five-hundred-twenty-seven'),
        ('vijfhonderdzesentwintig', 'vijfhonderdzevenentwintig'),
        ('all_cases[:511]', 'all_cases[:512]'),
        ('{UNKNOWN, TYPO}][:511]', '{UNKNOWN, TYPO}][:512]'),
        ('!= 511', '!= 512'),
        ('434, 435, 436, 437, 438', '434, 435, 436, 437, 438, 439'),
    ],
)

build(
    'validate-five-hundred-twenty-six-valid-list-cases.py',
    'validate-five-hundred-twenty-seven-valid-list-cases.py',
    [
        ('vijfhonderdzesentwintig', 'vijfhonderdzevenentwintig'),
        ('497, 498, 499, 500', '497, 498, 499, 500, 501'),
        ('all_cases[:511]', 'all_cases[:512]'),
        ('len(valid_cases) != 511', 'len(valid_cases) != 512'),
    ],
)

build(
    'validate-five-hundred-twenty-six-valid-mixed.py',
    'validate-five-hundred-twenty-seven-valid-mixed.py',
    [
        ('vijfhonderdzesentwintig', 'vijfhonderdzevenentwintig'),
        ('497, 498, 499, 500', '497, 498, 499, 500, 501'),
        ('][:511]', '][:512]'),
        ('len(valid_cases) != 511', 'len(valid_cases) != 512'),
    ],
)

verify_src = (ROOT / 'verify-five-hundred-twenty-six.py').read_text()
verify_text = verify_src.replace('five-hundred-twenty-six', 'five-hundred-twenty-seven')
(ROOT / 'verify-five-hundred-twenty-seven.py').write_text(verify_text)
print('verify-five-hundred-twenty-seven.py')
