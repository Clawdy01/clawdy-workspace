...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred-one'),
    ('driehonderdnegenenzestig', 'vierhonderdeen'),
    ('all_cases[:364]', 'all_cases[:396]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:396]'),
    ('!= 364', '!= 396'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred-one')
    text = src.read_text()
