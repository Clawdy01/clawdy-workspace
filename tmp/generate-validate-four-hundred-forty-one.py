...
repls = [
    ('three-hundred-sixty-one', 'four-hundred-forty-one'),
    ('driehonderdeenenzestig', 'vierhonderdeenenveertig'),
    ('all_cases[:356]', 'all_cases[:426]'),
    ('{UNKNOWN, TYPO}][:356]', '{UNKNOWN, TYPO}][:426]'),
    ('!= 356', '!= 426'),
...
for kind in ('valid-list-cases', 'valid-mixed'):
    ...
    dst = root / 'tmp' / f'validate-four-hundred-forty-one-{kind}.py'
    text = src.read_text()
