...
valid_cases = [case_name for case_name in all_cases if case_name not in {UNKNOWN, TYPO}][:245]
if len(valid_cases) != 245:
    raise SystemExit(f'kon geen tweehonderdzevenenveertig geldige casenamen vinden, kreeg {valid_cases}')
...
if expected_stderr not in (plain_proc.stderr or ''):
    raise SystemExit('plain stderr noemt niet alle tweehonderdzevenenveertig geldige first-seen cases')
