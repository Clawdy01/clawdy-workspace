...
repls = [
    ('four-hundred-sixty-three', 'four-hundred-eighty-one'),
    ('vierhonderddrieënzestig', 'vierhonderdeenentachtig'),
    ('[:448]', '[:466]'),
    ('!= 448', '!= 466'),
    ('kreeg 448', 'kreeg 466'),
]
for name in files:
    ...
    if name in {
        ...
        new = ', 443, 444, 445, 446, 447, 448, 449, 450, 451, 452, 453, 454, 455, 456, 457, 458]'
        if old not in text:
            raise SystemExit(f'mis expected ORDER tail in {name}')
        text = text.replace(old, new, 1)
    (root / name.replace('four-hundred-sixty-three', 'four-hundred-eighty-one')).write_text(text)
print('created')
