...
repls = [
    ('three-hundred-sixty-one', 'four-hundred-fifty-one'),
    ('driehonderdeenenzestig', 'vierhonderdeenenvijftig'),
    ('all_cases[:356]', 'all_cases[:436]'),
    ('{UNKNOWN, TYPO}][:356]', '{UNKNOWN, TYPO}][:436]'),
    ('!= 356', '!= 436'),
...
for kind in ('valid-list-cases', 'valid-mixed'):
    ...
    dst = root / 'tmp' / f'validate-four-hundred-fifty-one-{kind}.py'
    text = src.read_text()
