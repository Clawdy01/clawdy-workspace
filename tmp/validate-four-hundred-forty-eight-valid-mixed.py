...
valid_cases = [case_name for case_name in all_cases if case_name not in {UNKNOWN, TYPO}][:433]
if len(valid_cases) != 433:
    raise SystemExit(f'kon geen vierhonderdachtenveertig geldige casenamen vinden, kreeg {valid_cases}')
...
if expected_stderr not in (plain_proc.stderr or ''):
    raise SystemExit('plain stderr noemt niet alle vierhonderdachtenveertig geldige first-seen cases')
