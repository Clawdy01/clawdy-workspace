...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred-thirty-five'),
    ('driehonderdnegenenzestig', 'vierhonderdvijfendertig'),
    ('all_cases[:364]', 'all_cases[:420]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:420]'),
    ('!= 364', '!= 420'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred-thirty-five')
    text = src.read_text()
