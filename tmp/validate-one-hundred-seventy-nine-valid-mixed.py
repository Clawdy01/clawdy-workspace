...
valid_cases = [case_name for case_name in all_cases if case_name not in {UNKNOWN, TYPO}][:179]
if len(valid_cases) != 179:
    raise SystemExit(f'kon geen honderdnegenenzeventig geldige casenamen vinden, kreeg {valid_cases}')
...
if expected_stderr not in (plain_proc.stderr or ''):
    raise SystemExit('plain stderr noemt niet alle honderdnegenenzeventig geldige first-seen cases')
