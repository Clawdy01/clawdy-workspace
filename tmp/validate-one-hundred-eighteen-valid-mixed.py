...
valid_cases = [case_name for case_name in all_cases if case_name not in {UNKNOWN, TYPO}][:118]
if len(valid_cases) != 118:
    raise SystemExit(f'kon geen honderdachttien geldige casenamen vinden, kreeg {valid_cases}')
...
if expected_stderr not in (plain_proc.stderr or ''):
    raise SystemExit('plain stderr noemt niet alle honderdachttien geldige first-seen cases')
