...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred-fifty-four'),
    ('driehonderdnegenenzestig', 'vierhonderdvierenvijftig'),
    ('all_cases[:364]', 'all_cases[:439]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:439]'),
    ('!= 364', '!= 439'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred-fifty-four')
    text = src.read_text()
