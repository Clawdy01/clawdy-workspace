...
repls = [
    ('four-hundred-fifty-eight', 'four-hundred-sixty-five'),
    ('vierhonderdachtenvijftig', 'vierhonderdvijfenzestig'),
    ('all_cases[:443]', 'all_cases[:450]'),
    ('{UNKNOWN, TYPO}][:443]', '{UNKNOWN, TYPO}][:450]'),
    ('!= 443', '!= 450'),
    ('kreeg 443', 'kreeg 450'),
...
for src_name in [
    ...
    src = root / src_name
    dst = root / src_name.replace('four-hundred-fifty-eight', 'four-hundred-sixty-five')
    text = src.read_text()
