...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred-fifty-nine'),
    ('driehonderdnegenenzestig', 'vierhonderdnegenenvijftig'),
    ('all_cases[:364]', 'all_cases[:444]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:444]'),
    ('!= 364', '!= 444'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred-fifty-nine')
    text = src.read_text()
