...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred-two'),
    ('driehonderdnegenenzestig', 'vierhonderdtwee'),
    ('all_cases[:364]', 'all_cases[:397]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:397]'),
    ('!= 364', '!= 397'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred-two')
    text = src.read_text()
