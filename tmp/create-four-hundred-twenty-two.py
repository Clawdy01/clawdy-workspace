...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred-twenty-two'),
    ('driehonderdnegenenzestig', 'vierhonderdtweeentwintig'),
    ('all_cases[:364]', 'all_cases[:407]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:407]'),
    ('!= 364', '!= 407'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred-twenty-two')
    text = src.read_text()
