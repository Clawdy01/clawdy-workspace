...
valid_cases = [case_name for case_name in all_cases if case_name not in {UNKNOWN, TYPO}][:356]
if len(valid_cases) != 356:
    raise SystemExit(f'kon geen driehonderdeenenzestig geldige casenamen vinden, kreeg {valid_cases}')
...
if expected_stderr not in (plain_proc.stderr or ''):
    raise SystemExit('plain stderr noemt niet alle driehonderdeenenzestig geldige first-seen cases')
