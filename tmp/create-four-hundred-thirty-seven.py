...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred-thirty-seven'),
    ('driehonderdnegenenzestig', 'vierhonderdzevenendertig'),
    ('all_cases[:364]', 'all_cases[:422]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:422]'),
    ('!= 364', '!= 422'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred-thirty-seven')
    text = src.read_text()
