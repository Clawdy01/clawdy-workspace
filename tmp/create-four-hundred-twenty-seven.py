...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred-twenty-seven'),
    ('driehonderdnegenenzestig', 'vierhonderdzevenentwintig'),
    ('all_cases[:364]', 'all_cases[:412]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:412]'),
    ('!= 364', '!= 412'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred-twenty-seven')
    text = src.read_text()
