...
ORDER = [27, 26, 23, 25, 21, 24, 19, 22, 17, 20, 15, 18, 13, 16, 11, 14, 9, 12, 7, 10, 5, 8, 3, 6, 1, 4, 0, 2]
...
valid_cases = [case_name for case_name in all_cases if case_name not in {UNKNOWN, TYPO}][:28]
if len(valid_cases) != 28:
    raise SystemExit(f'kon geen achtentwintig geldige casenamen vinden, kreeg {valid_cases}')
...
if expected_stderr not in (plain_proc.stderr or ''):
    raise SystemExit('plain stderr noemt niet alle achtentwintig geldige first-seen cases')
