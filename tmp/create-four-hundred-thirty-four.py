...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred-thirty-four'),
    ('driehonderdnegenenzestig', 'vierhonderdvierendertig'),
    ('all_cases[:364]', 'all_cases[:419]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:419]'),
    ('!= 364', '!= 419'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred-thirty-four')
    text = src.read_text()
