...
repls = [
    ('three-hundred-sixty-nine', 'three-hundred-ninety-five'),
    ('driehonderdnegenenzestig', 'driehonderdvijfennegentig'),
    ('all_cases[:364]', 'all_cases[:390]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:390]'),
    ('!= 364', '!= 390'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'three-hundred-ninety-five')
    text = src.read_text()
