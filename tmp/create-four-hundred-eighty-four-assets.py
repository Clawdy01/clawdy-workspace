...
repls = [
    ('four-hundred-sixty-three', 'four-hundred-eighty-four'),
    ('vierhonderddrieënzestig', 'vierhonderdvierentachtig'),
    ('[:448]', '[:469]'),
    ('!= 448', '!= 469'),
    ('kreeg 448', 'kreeg 469'),
]
for name in files:
    ...
    if name in {
        ...
        new = ', 443, 444, 445, 446, 447, 448, 449, 450, 451, 452, 453, 454, 455, 456, 457, 458, 459, 460, 461]'
        if old not in text:
            raise SystemExit(f'mis expected ORDER tail in {name}')
        text = text.replace(old, new, 1)
    (root / name.replace('four-hundred-sixty-three', 'four-hundred-eighty-four')).write_text(text)
print('created')
