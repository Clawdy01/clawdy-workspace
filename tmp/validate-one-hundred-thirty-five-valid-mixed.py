...
valid_cases = [case_name for case_name in all_cases if case_name not in {UNKNOWN, TYPO}][:135]
if len(valid_cases) != 135:
    raise SystemExit(f'kon geen honderdvijfendertig geldige casenamen vinden, kreeg {valid_cases}')
...
if expected_stderr not in (plain_proc.stderr or ''):
    raise SystemExit('plain stderr noemt niet alle honderdvijfendertig geldige first-seen cases')
