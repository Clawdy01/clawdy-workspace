...
valid_cases = [case_name for case_name in all_cases if case_name not in {UNKNOWN, TYPO}][:459]
if len(valid_cases) != 459:
    raise SystemExit(f'kon geen vierhonderdvierenzeventig geldige casenamen vinden, kreeg {valid_cases}')
...
if expected_stderr not in (plain_proc.stderr or ''):
    raise SystemExit('plain stderr noemt niet alle vierhonderdvierenzeventig geldige first-seen cases')
