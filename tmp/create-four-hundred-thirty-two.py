...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred-thirty-two'),
    ('driehonderdnegenenzestig', 'vierhonderdtweeendertig'),
    ('all_cases[:364]', 'all_cases[:417]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:417]'),
    ('!= 364', '!= 417'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred-thirty-two')
    text = src.read_text()
