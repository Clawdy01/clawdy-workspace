...
for name in files:
    ...
    text = text.replace('all_cases[:390]', 'all_cases[:392]')
    text = text.replace('{UNKNOWN, TYPO}][:390]', '{UNKNOWN, TYPO}][:392]')
    text = text.replace('!= 390', '!= 392')
    text = text.replace('kreeg 390', 'kreeg 392')
    text = text.replace(' 390)', ' 392)')
    if 'ORDER =' in text:
        text = text.replace(', 388, 389]', ', 388, 389, 390, 391]')
