...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred-forty-three'),
    ('driehonderdnegenenzestig', 'vierhonderddrieenveertig'),
    ('all_cases[:364]', 'all_cases[:428]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:428]'),
    ('!= 364', '!= 428'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred-forty-three')
    text = src.read_text()
