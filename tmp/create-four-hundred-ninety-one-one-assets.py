...
repls = [
    ('four-hundred-sixty-three', 'four-hundred-ninety-one-one'),
    ('vierhonderddrieënzestig', 'vierhonderdtweeënnegentig'),
    ('[:448]', '[:477]'),
    ('!= 448', '!= 477'),
    ('kreeg 448', 'kreeg 477'),
]
for name in files:
    ...
    if name in {
        ...
        new = ', 443, 444, 445, 446, 447, 448, 449, 450, 451, 452, 453, 454, 455, 456, 457, 458, 459, 460, 461, 462, 463, 464, 465, 466, 467, 468, 469]'
        if old not in text:
            raise SystemExit(f'mis expected ORDER tail in {name}')
        text = text.replace(old, new, 1)
    (root / name.replace('four-hundred-sixty-three', 'four-hundred-ninety-one-one')).write_text(text)
print('created')
