...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred-fifty-three'),
    ('driehonderdnegenenzestig', 'vierhonderddrieënvijftig'),
    ('all_cases[:364]', 'all_cases[:438]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:438]'),
    ('!= 364', '!= 438'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred-fifty-three')
    text = src.read_text()
