...
valid_cases = [case_name for case_name in all_cases if case_name not in {UNKNOWN, TYPO}][:589]
if len(valid_cases) != 589:
    raise SystemExit(f'kon geen zeshonderdvier geldige casenamen vinden, kreeg {valid_cases}')
...
if expected_stderr not in (plain_proc.stderr or ''):
    raise SystemExit('plain stderr noemt niet alle vijfhonderdnegenentachtig geldige first-seen cases')
