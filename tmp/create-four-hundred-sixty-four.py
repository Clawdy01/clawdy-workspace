...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred-sixty-four'),
    ('driehonderdnegenenzestig', 'vierhonderdvierenzestig'),
    ('all_cases[:364]', 'all_cases[:449]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:449]'),
    ('!= 364', '!= 449'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred-sixty-four')
    text = src.read_text()
