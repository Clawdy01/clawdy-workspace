...
valid_cases = [case_name for case_name in all_cases if case_name not in {UNKNOWN, TYPO}][:204]
if len(valid_cases) != 204:
    raise SystemExit(f'kon geen tweehonderdvijf geldige casenamen vinden, kreeg {valid_cases}')
...
if expected_stderr not in (plain_proc.stderr or ''):
    raise SystemExit('plain stderr noemt niet alle tweehonderdvijf geldige first-seen cases')
