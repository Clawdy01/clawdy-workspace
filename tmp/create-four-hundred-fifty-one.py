...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred-fifty-one'),
    ('driehonderdnegenenzestig', 'vierhonderdeenenvijftig'),
    ('all_cases[:364]', 'all_cases[:436]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:436]'),
    ('!= 364', '!= 436'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred-fifty-one')
    text = src.read_text()
