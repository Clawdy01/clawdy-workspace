...
valid_cases = [case_name for case_name in all_cases if case_name not in {UNKNOWN, TYPO}][:619]
if len(valid_cases) != 619:
    raise SystemExit(f'kon geen zeshonderdvierendertig geldige casenamen vinden, kreeg {valid_cases}')
...
if expected_stderr not in (plain_proc.stderr or ''):
    raise SystemExit('plain stderr noemt niet alle zeshonderdnegentien geldige first-seen cases')
