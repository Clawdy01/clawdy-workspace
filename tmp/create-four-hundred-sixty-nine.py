...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred-sixty-nine'),
    ('driehonderdnegenenzestig', 'vierhonderdnegenenzestig'),
    ('all_cases[:364]', 'all_cases[:454]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:454]'),
    ('!= 364', '!= 454'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred-sixty-nine')
    text = src.read_text()
