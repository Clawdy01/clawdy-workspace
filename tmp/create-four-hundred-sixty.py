...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred-sixty'),
    ('driehonderdnegenenzestig', 'vierhonderdzestig'),
    ('all_cases[:364]', 'all_cases[:445]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:445]'),
    ('!= 364', '!= 445'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred-sixty')
    text = src.read_text()
