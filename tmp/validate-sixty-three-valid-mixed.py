...
ORDER = [58, 57, 56, 55, 54, 53, 52, 51, 50, 49, 48, 47, 46, 45, 44, 43, 42, 41, 40, 39, 38, 37, 36, 35, 34, 33, 32, 31, 30, 29, 26, 28, 24, 27, 22, 25, 20, 23, 18, 21, 16, 19, 14, 17, 12, 15, 10, 13, 8, 11, 6, 9, 4, 7, 2, 5, 0, 3, 1, 59, 60, 61, 62]
...
valid_cases = [case_name for case_name in all_cases if case_name not in {UNKNOWN, TYPO}][:63]
if len(valid_cases) != 63:
    raise SystemExit(f'kon geen drieënzestig geldige casenamen vinden, kreeg {valid_cases}')
...
if expected_stderr not in (plain_proc.stderr or ''):
    raise SystemExit('plain stderr noemt niet alle drieënzestig geldige first-seen cases')
