...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred-seventeen'),
    ('driehonderdnegenenzestig', 'vierhonderdzeventien'),
    ('all_cases[:364]', 'all_cases[:402]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:402]'),
    ('!= 364', '!= 402'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred-seventeen')
    text = src.read_text()
