...
repls = [
    ('four-hundred-sixty-three', 'four-hundred-seventy-eight'),
    ('vierhonderddrieënzestig', 'vierhonderdachtenzeventig'),
    ('[:448]', '[:463]'),
    ('!= 448', '!= 463'),
    ('kreeg 448', 'kreeg 463'),
]
for name in files:
    ...
    if name in {
        ...
        new = ', 443, 444, 445, 446, 447, 448, 449, 450, 451, 452, 453, 454, 455, 456]'
        if old not in text:
            raise SystemExit(f'mis expected ORDER tail in {name}')
        text = text.replace(old, new, 1)
    (root / name.replace('four-hundred-sixty-three', 'four-hundred-seventy-eight')).write_text(text)
print('created')
