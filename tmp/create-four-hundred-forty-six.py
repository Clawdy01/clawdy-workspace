...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred-forty-six'),
    ('driehonderdnegenenzestig', 'vierhonderdzesenveertig'),
    ('all_cases[:364]', 'all_cases[:431]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:431]'),
    ('!= 364', '!= 431'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred-forty-six')
    text = src.read_text()
