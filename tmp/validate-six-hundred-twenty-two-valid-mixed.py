...
valid_cases = [case_name for case_name in all_cases if case_name not in {UNKNOWN, TYPO}][:607]
if len(valid_cases) != 607:
    raise SystemExit(f'kon geen zeshonderdtweeëntwintig geldige casenamen vinden, kreeg {valid_cases}')
...
if expected_stderr not in (plain_proc.stderr or ''):
    raise SystemExit('plain stderr noemt niet alle zeshonderdzeven geldige first-seen cases')
