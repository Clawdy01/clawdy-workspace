...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred-forty-nine'),
    ('driehonderdnegenenzestig', 'vierhonderdnegenenveertig'),
    ('all_cases[:364]', 'all_cases[:434]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:434]'),
    ('!= 364', '!= 434'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred-forty-nine')
    text = src.read_text()
