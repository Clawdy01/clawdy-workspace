...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred-fifty-eight'),
    ('driehonderdnegenenzestig', 'vierhonderdachtenvijftig'),
    ('all_cases[:364]', 'all_cases[:443]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:443]'),
    ('!= 364', '!= 443'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred-fifty-eight')
    text = src.read_text()
