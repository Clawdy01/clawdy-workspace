...
valid_cases = [case_name for case_name in all_cases if case_name not in {UNKNOWN, TYPO}][:114]
if len(valid_cases) != 114:
    raise SystemExit(f'kon geen honderdveertien geldige casenamen vinden, kreeg {valid_cases}')
...
if expected_stderr not in (plain_proc.stderr or ''):
    raise SystemExit('plain stderr noemt niet alle honderdveertien geldige first-seen cases')
