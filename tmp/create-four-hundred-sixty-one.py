...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred-sixty-one'),
    ('driehonderdnegenenzestig', 'vierhonderdeenenzestig'),
    ('all_cases[:364]', 'all_cases[:446]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:446]'),
    ('!= 364', '!= 446'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred-sixty-one')
    text = src.read_text()
