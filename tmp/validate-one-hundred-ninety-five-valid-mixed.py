...
valid_cases = [case_name for case_name in all_cases if case_name not in {UNKNOWN, TYPO}][:194]
if len(valid_cases) != 194:
    raise SystemExit(f'kon geen honderdvijfennegentig geldige casenamen vinden, kreeg {valid_cases}')
...
if expected_stderr not in (plain_proc.stderr or ''):
    raise SystemExit('plain stderr noemt niet alle honderdvijfennegentig geldige first-seen cases')
