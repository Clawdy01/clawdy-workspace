...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred-forty-four'),
    ('driehonderdnegenenzestig', 'vierhonderdvierenveertig'),
    ('all_cases[:364]', 'all_cases[:429]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:429]'),
    ('!= 364', '!= 429'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred-forty-four')
    text = src.read_text()
