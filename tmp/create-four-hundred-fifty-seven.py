...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred-fifty-seven'),
    ('driehonderdnegenenzestig', 'vierhonderdzevenenvijftig'),
    ('all_cases[:364]', 'all_cases[:442]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:442]'),
    ('!= 364', '!= 442'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred-fifty-seven')
    text = src.read_text()
