...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred-twenty'),
    ('driehonderdnegenenzestig', 'vierhonderdtwintig'),
    ('all_cases[:364]', 'all_cases[:405]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:405]'),
    ('!= 364', '!= 405'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred-twenty')
    text = src.read_text()
