...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred-forty-two'),
    ('driehonderdnegenenzestig', 'vierhonderdtweeenveertig'),
    ('all_cases[:364]', 'all_cases[:427]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:427]'),
    ('!= 364', '!= 427'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred-forty-two')
    text = src.read_text()
