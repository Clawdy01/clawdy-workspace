...
repls = [
    ('three-hundred-sixty-one', 'four-hundred-forty'),
    ('driehonderdeenenzestig', 'vierhonderdveertig'),
    ('all_cases[:356]', 'all_cases[:425]'),
    ('{UNKNOWN, TYPO}][:356]', '{UNKNOWN, TYPO}][:425]'),
    ('!= 356', '!= 425'),
...
for kind in ('valid-list-cases', 'valid-mixed'):
    ...
    dst = root / 'tmp' / f'validate-four-hundred-forty-{kind}.py'
    text = src.read_text()
