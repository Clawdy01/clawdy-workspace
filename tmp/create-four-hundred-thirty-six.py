...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred-thirty-six'),
    ('driehonderdnegenenzestig', 'vierhonderdzesendertig'),
    ('all_cases[:364]', 'all_cases[:421]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:421]'),
    ('!= 364', '!= 421'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred-thirty-six')
    text = src.read_text()
