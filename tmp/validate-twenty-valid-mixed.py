...
ORDER = [19, 16, 18, 14, 17, 12, 15, 10, 13, 8, 11, 6, 9, 4, 7, 2, 5, 1, 3, 0]
...
valid_cases = [case_name for case_name in all_cases if case_name not in {UNKNOWN, TYPO}][:20]
if len(valid_cases) != 20:
    raise SystemExit(f'kon geen twintig geldige casenamen vinden, kreeg {valid_cases}')
...
if expected_stderr not in (plain_proc.stderr or ''):
    raise SystemExit('plain stderr noemt niet alle twintig geldige first-seen cases')
