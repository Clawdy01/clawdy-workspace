...
valid_cases = [case_name for case_name in all_cases if case_name not in {UNKNOWN, TYPO}][:177]
if len(valid_cases) != 177:
    raise SystemExit(f'kon geen honderdzevenenzeventig geldige casenamen vinden, kreeg {valid_cases}')
...
if expected_stderr not in (plain_proc.stderr or ''):
    raise SystemExit('plain stderr noemt niet alle honderdzevenenzeventig geldige first-seen cases')
