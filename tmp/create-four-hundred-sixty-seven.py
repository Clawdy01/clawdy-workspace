...
repls = [
    ('three-hundred-sixty-nine', 'four-hundred-sixty-seven'),
    ('driehonderdnegenenzestig', 'vierhonderdzevenenzestig'),
    ('all_cases[:364]', 'all_cases[:452]'),
    ('{UNKNOWN, TYPO}][:364]', '{UNKNOWN, TYPO}][:452]'),
    ('!= 364', '!= 452'),
...
for name in (
    ...
    src = root / 'tmp' / name
    dst = root / 'tmp' / name.replace('three-hundred-sixty-nine', 'four-hundred-sixty-seven')
    text = src.read_text()
