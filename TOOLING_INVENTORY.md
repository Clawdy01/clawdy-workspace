...
- `scripts/media-sanity-check.py` — snelle inspectie van video/audio/image-output, inclusief JSON/JSONL, preset/fail-profielen, kind/exclude filtering, threshold-warnings, directory/batch-check via `--dir`, CI-vriendelijke fail-modes, timestamped rapport-output, append-mode en summary-only rapporten
