...
(async()=>{
  ...
  const btn = page.locator('button[aria-haspopup="dialog"]').filter({ hasText: /^\s*Github\s*$/i }).first();
  await btn.click({ force: true }).catch(()=>null);
  await page.waitForTimeout(2000);
  const maybeReveal = page.locator('button, a').filter({ hasText: /show|reveal|view/i });
  const count = await maybeReveal.count();
  for (let i=0;i<count;i++) {
    const b=maybeReveal.nth(i);
    const txt=(await b.textContent().catch(()=>''))||'';
    if (/password/i.test(txt) || /show/i.test(txt) || /reveal/i.test(txt)) await b.click().catch(()=>null);
  }
  await page.waitForTimeout(1000);
  const data = await page.evaluate(() => {
    const dialog = document.querySelector('[role="dialog"], dialog, cdk-dialog-container');
    const root = dialog || document;
    return {
      text: (root.innerText || '').slice(0,5000),
      inputs: [...root.querySelectorAll('input')].map(e => ({type:e.type||'', id:e.id||'', name:e.name||'', value:e.value||'', outer:e.outerHTML.slice(0,500)})),
      buttons: [...root.querySelectorAll('button,a')].map(e => ({text:(e.innerText||'').trim(), aria:e.getAttribute('aria-label')||'', outer:e.outerHTML.slice(0,400)})).filter(x => x.text || x.aria).slice(0,100)
    }
  });
