...
(async () => {
  ...
  const data = await page.evaluate(() => {
    ...
    return items.filter(x => /username|proton|create|email|free|account|current/i.test(`${x.id} ${x.text} ${x.ariaLabel} ${x.testId}`));
  });

  const screenshot = path.join(OUT_DIR, 'proton-toggle-email-mode.png');
  await page.screenshot({ path: screenshot, fullPage: true });
  const result = { checkedAt: new Date().toISOString(), url: page.url(), data, screenshot };
  fs.writeFileSync(path.join(OUT_DIR, 'proton-toggle-email-mode.json'), JSON.stringify(result, null, 2));
