...
(async()=>{
  ...
  if (!email || !password) {
    console.error('usage: bitwarden_full_login_search.js <email> <password> [searchTerm]');
    process.exit(2);
  }
  ...
  try {
    ...
    if (page.url().includes('/device-verification')) {
      ...
      if (await codeInput.count()) {
        await codeInput.click().catch(() => null);
        await codeInput.fill(code).catch(() => null);
        await codeInput.press('Tab').catch(() => null);
    ...
    if (await search.count()) {
      ...
      out.ok = true;
      out.searchBody = ((await page.textContent('body')) || '').slice(0, 5000);
    }
  } finally {
