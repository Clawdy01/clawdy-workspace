...
(async()=>{
  ...
  console.log('btnCount', await btn.count());
  if (await btn.count()) {
    console.log('before', await btn.evaluate(el => ({text: el.innerText, html: el.outerHTML.slice(0,800)})).catch(()=>null));
    await btn.click({ force: true }).catch(e => console.log('clickErr', String(e)));
    await page.waitForTimeout(2500);
    console.log('after url', page.url());
    const data = await page.evaluate(() => ({
      dialogs:[...document.querySelectorAll('[role="dialog"], dialog')].map(el => ({text:(el.innerText||'').slice(0,2000), html:el.outerHTML.slice(0,1200)})).slice(0,10),
      drawer:[...document.querySelectorAll('*')].filter(el => (el.innerText||'').includes('clawdy01@christiandekok.nl') && (el.innerText||'').includes('Github')).map(el=>({tag:el.tagName, text:(el.innerText||'').slice(0,500), html:el.outerHTML.slice(0,1200)})).slice(0,10),
