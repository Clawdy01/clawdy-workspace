...
- Bewijspad voor de huidige AI-briefing-config is nu expliciet geactiveerd en opnieuw gecontroleerd: `python3 scripts/ai-briefing-status.py --json`, `python3 scripts/ai-briefing-watchdog.py --json` en `python3 scripts/ai-briefing-watchdog-alert.py --mode proof-check` tonen dat de job gezond is, dat de huidige config nog `0/3` gekwalificeerde runs heeft, dat de eerstvolgende kwalificatierun op `2026-04-17 09:00 CEST` staat en dat het bewijsdoel bij groene runs op `2026-04-19 09:15 CEST` ligt. Daarnaast is de watchdog-output nu opnieuw naar de standaard consumer-artifacts gepubliceerd via `python3 scripts/ai-briefing-watchdog.py --require-qualified-runs 3 --consumer-bundle board-suite --consumer-format json --json`, waardoor `tmp/ai-briefing/reports/ai-briefing-watchdog.{json,txt,jsonl}` de actuele proving-status bevatten. Er is dus geen harde blocker, alleen gepland wachten op de komende runs.
- `scripts/ai-briefing-watchdog-producer.py` heeft nu vaste proving-modes `proof-board`, `proof-eventlog` en `proof-all`, zodat dezelfde `require-qualified-runs=3` board/eventlog-publicatie zonder losse handmatige flags herhaalbaar is; `python3 scripts/ai-briefing-watchdog-producer.py proof-all --quiet` is direct geverifieerd en publiceerde de huidige `0/3` proving-status opnieuw naar de standaard artifacts.
